...
- [ ] M9 — Polish and ship (plan: `docs/plans/2026-09-18-m9-polish-and-ship.md`; Jeremy added a Section 508 accessibility review and a DoD-oriented security review up front)
  ...
  - [x] Device 2026-09-18: the M9 batch compiled and ran; launch sweep removed 2 leftover files; speaker labelling failed in the background (GPU not permitted) → models now load on CPU + Neural Engine and a background failure waits for the foreground. Re-test: stop a recording, leave the app immediately, come back after a minute → speakers labelled without Retry
