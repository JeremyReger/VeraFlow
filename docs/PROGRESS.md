...
- [x] Large text sizes across the design system (2026-09-20, from Jeremy's iPhone screenshots)
  - [x] Jeremy's screenshots answered the accessibility-size check from the Ask-pill item above, and the answer was no: the strip read "SU MM A… / TR AN S… / AU DIO / AS K", and the Ask capsule wasn't drawing as a capsule at all. Three labels weren't enough, because tracked capitals at an accessibility size are wider than an iPhone no matter how few there are
  - [x] `VFTabs` picks its layout with `ViewThatFits` rather than a size threshold: the strip is used exactly as it was whenever it fits, and scrolls sideways when it doesn't. Whether four tracked-capital labels fit depends on the text size, the device width and the words, so it's measured, not guessed. Labels are one line at their natural width, so none can break mid-word again
  - [x] Each `ViewThatFits` candidate has its own `@Namespace` for the underline: the candidates are all built in order to be measured, and two live views claiming one matched-geometry id is a runtime warning — the same class of bug as the LazyVStack id collision above
  - [x] The strip's own labels stop growing at `.accessibility1` (`VFTabMetrics.maximumLabelSize`, about 1.7x the default). Nothing else is clamped — the transcript, summary and action items scale the whole way. The clamp is only so that reaching Audio doesn't mean scrolling past two screens of "TRANSCRIPT"
  - [x] The real cause of the missing pill was `.frame(height: 30)`: a fixed height can't grow, so a taller label hangs out of its own capsule. The same bug was in five other places — `VFPrimaryPillStyle`, `VFSecondaryPillStyle`, `VFChip`, `VFLiveChip` and the NAME? chip — all now vertical padding plus a minimum, which leaves every one of them pixel-identical at the default text size
  - [x] `VFPickerRowLabel` ("MICROPHONE / iPhone Microphone") had horizontal padding and no vertical padding, so once the eyebrow wrapped, the lines sat on the card's border and read as overlapping it. It has vertical padding now, the value wraps to two lines instead of truncating, and the glyphs scale with the text
