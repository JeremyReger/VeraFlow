...
- Summary section editing: the overview and every plain list block (decisions, open questions, client goals, concerns, next meeting, location, customer requests, issues found, quote notes) can be rewritten, added to and cleared from a pencil in the section heading, with "Add a section" at the foot for what the model left blank. The edits are an overlay (`SummaryEdits`), so the model's own output stays readable and a re-run starts clean. Key points and work areas render as groups and are not editable yet.
...
- **User edits (v1.1):** action items live in `SummaryRecord.actionItemsState`, prose in `SummaryRecord.summaryEditsJSON` (`SummaryEdits`: replacements by line index, added lines, replaced paragraphs; a block inside a subject or work area is keyed `field#index`, addressing the stored `topics`/`areas` array). Both are overlays — `payloadJSON` is never written to — and `resolvedPayload()` applies both, so exports, Reminders, the library card and library search read the user's wording while the model's version stays with the summary. The overlay belongs to the record it was made on, so a re-run starts clean. Editing a key-point subject also rebuilds the flat `keyPoints` list the summarizer derives from the subjects, so exports and search stay in step. Measurements and materials are structured and verbatim and have no overlay at all.
