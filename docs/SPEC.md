...
- **User edits (v1.1):** action items live in `SummaryRecord.actionItemsState`, prose in `SummaryRecord.summaryEditsJSON` (`SummaryEdits`: replacements by line index, added lines, replaced paragraphs). Both are overlays — `payloadJSON` is never written to — and `resolvedPayload()` applies both, so exports, Reminders, the library card and library search read the user's wording while the model's version stays with the summary. The overlay belongs to the record it was made on, so a re-run starts clean.
